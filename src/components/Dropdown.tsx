...
const Dropdown = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div className="h-max w-[15rem]">
      ...
      <motion.ul
        initial={{ y: "-150%", zIndex: -99 }}
        animate={{ y: showMenu ? "0%" : "-150%" }}
        transition={{ duration: 0.5 }}
        className="absolute  text-sm text-gray-700 dark:text-gray-200 rounded-b-lg shadow dark:bg-gray-700 h-max max-h-[8.5rem] w-[15rem]"
      >
        {children}
      </motion.ul>
    </div>
